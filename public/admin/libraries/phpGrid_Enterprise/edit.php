...
if($oper != ''){
    ...
    switch($oper){
        case 'add':
			$arrFields['created_at'] = $arrFields['updated_at'] = time();
			$arrFields['created_by'] = $arrFields['updated_by'] = $_COOKIE['current_user_id'];
			/*if(array_key_exists("branch_id", $arrFields) && !$arrFields['branch_id']){
				$CI->load->library('m_database');
				$user_id = $CI->session->userdata('user_id_session');
				$branch_id =  $CI->m_database->fieldQuery("SELECT branch_id FROM users where user_id = " . $user_id, "branch_id");
				$arrFields['branch_id'] = $branch_id;
			}*/
            $sqlCrud = $db->GetInsertSQL($rs, $arrFields, $sql_table);
            break;
        case 'edit':
			$arrFields['updated_at'] = time();
			$arrFields['updated_by'] = $_COOKIE['current_user_id'];
