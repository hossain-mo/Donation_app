...
//    private $_file_path;
    ...
    public function __construct($sql, $sql_key=array(), $sql_table='', $db_connection= array()){
        ...
        $this->_ver_num             = 'phpGrid(v7.2.6) {free-jqGrid:v4.13.6, jQuery:v2.1.4, jQuery UI:1.11.2}';
        $this->sys_msg              = null;
        $this->form_width            = 450;
        ...
        $this->edit_file            = ABS_PATH .'/edit.php';
    ...
    public function set_colNames($results){
		...
		$col_names = array_map(array($this, 'modify_titles'), $col_names);
        ...
        for($i=0;$i<$this->_num_fields;$i++){
            ...
            if(($col_type=='D'||$col_type=='T') && !in_array($col_name, $this->col_readonly)){   // do not display datepicker/datetimepicker if readonly
                if((isset($this->col_formats[$col_name]['date']))){
                    ...
                    $editoptions['dataInit'] = '###function(el){$(el).datetimepicker({
                                                showMeridian: true,
                todayHighlight: true,
                autoclose: true,format:\''. $this->col_formats[$col_name]['datetime']['datePickerFormat']  .'\', lang:\''. ($this->locale=='cn'?'ch':$this->locale) .'\'});}###';
                }elseif((isset($this->col_formats[$col_name]['time']))) {
                    $editoptions['dataInit'] = '###function(el){$(el).timepicker();}###';
                }else{
                    $editoptions['dataInit'] = '###function(el){$(el).datepicker({
            ...
            if(isset($this->col_formats[$col_name])){
                if(isset($this->col_formats[$col_name]['link'])){
                    ...
                    $cols['formatoptions'] = $formatoptions;
                    $cols['searchoptions'] = array('dataInit' => '###function(el){$(el).datepicker({
                                                changeMonth: true, changeYear: true,dateFormat:\''. $this->col_formats[$col_name]['date']['datePickerFormat']      .'\', 
                                                onSelect: function(dateText, inst){ $(\'#'. $this->jq_gridName .'\')[0].triggerToolbar();}},$.datepicker.regional[\''. $this->locale .'\']);}###');
                }elseif(isset($this->col_formats[$col_name]['datetime'])){
                    $cols['formatter'] = 'date';
                    $formatoptions = array();
                    ...
                    $cols['formatter'] = 'date';
                    $formatoptions = array();
                    $formatoptions['srcformat']            = $this->col_formats[$col_name]['time']['srcformat'];
                    $formatoptions['newformat']            = $this->col_formats[$col_name]['time']['newformat'];
                    $cols['formatoptions'] = $formatoptions;
                ...
                $cols['stype'] = 'select';
                $cols['searchoptions'] = array('sopt'=>array('eq'), 'value'=>':All;'. str_replace('null:;', '', $this->col_edittypes[$col_name]['value']),
                "dataInit"=>"###function (el) { $(el).select2();}###");
            }
                        ...
                        $editEvtHanlder .= '
                            ...
                            if(file_name=="" || file_name === null || (typeof file_name == "undefined")){
                                ...
                                if(file_name.startsWith("<img")){
									href = $(file_name).attr("src");
								}else{ 
									href = "'. $base_url .'" + file_name;
								}
                                $("#_fileLink").attr("href", href).html(file_name).show();
                                $("#_btnFileDelete").show();
                                $("#'. $col_file .'").hide();
        ...
        if($this->obj_md != null){
            ...
            for($i=0;$i<count($this->obj_md);$i++){
                $md_onselectrow .= 'var mgrid = $("#'.$this->jq_gridName .'");
                            ...
                            // console.log(fkey_value);
                            jQuery("#'. $this->obj_md[$i]->get_jq_gridName() .'").jqGrid("setGridParam", {editurl:"'. $this->edit_file .'?dt='. $this->jq_datatype .'&gn='. $this->obj_md[$i]->get_jq_gridName() .'&src=md&fkey='.$this->obj_md[$i]->get_sql_mfkey().'&fkey_value="+fkey_value});'."\n";
        ...
        switch($this->edit_mode){
            ...
            case 'INLINE':
                ...
                echo '{            // --------- add options ---------
                                ...
                                beforeShowForm: function(frm) {$(this).resetSelection();';
                foreach($this->col_readonly as $key => $value){ echo 'if($("#'. $value .'").prev().prop("tagName") == "select"){ 
				...
				// ------ commented and edited by raafat for before show add form
				//echo $this->get_beforeShowForm_readonlyattr();
				/*echo 'beforeShowForm: function(frm) {$(this).resetSelection();';
                foreach($this->col_readonly as $key => $value){ echo 'if($("#'. $value .'").prev().prop("tagName") == "select"){ 
                                $("#'. $value .'").removeAttr("readonly");
                            } else { 
                                $("#'. $value .'").removeAttr("disabled"); 
            ...
            echo 'jQuery("#'. $this->jq_gridName.'")
                ...
                .navButtonAdd('.$this->jq_pagerName.',{
                    ...
                    onClickButton: function(){
                        ...
                        $.ajax({
                              url: "'. $this->edit_file .'?dt='. $this->jq_datatype .'&gn='.$this->jq_gridName .'",
                              type: "POST",
                              data: rowData,
                              dataType: "json",
...
//        }
    ...
    public function enable_edit($edit_mode = 'FORM', $options='CRUD', $edit_file= null){
		if($edit_file == null){
        ...
        switch($edit_mode)    {
            ...
            case 'INLINE':
                ...
                $this->set_jq_editurl($edit_file .'?dt='. $this->jq_datatype .'&gn='.$this->jq_gridName);
    ...
    public function set_col_date($col_name, $srcformat="Y-m-d", $newformat="Y-m-d", $datePickerFormat="yy-mm-dd"){
     ...
     */
    public function set_col_datetime($col_name, $srcformat="Y-m-d", $newformat="Y-m-d", $datePickerFormat="yy-mm-dd"){
    ...
    public function set_col_time($col_name, $srcformat="H:i", $newformat="h:i A", $datePickerFormat="H:i"){
        $this->col_formats[$col_name]['time'] = array("srcformat"=>$srcformat,
            "newformat"=>$newformat,
            "datePickerFormat"=>$datePickerFormat);
...
LOADCOMPLETE;
    ...
    public function set_masterdetail($obj_grid, $fkey, $m_fkey=-1){
        ...
        if($obj_grid instanceof C_DataGrid){
            ...
            $obj_grid->set_jq_editurl($this->edit_file .'?dt='. $this->jq_datatype .'&gn='. $obj_grid->jq_gridName .'&src=md&fkey='. $m_fkey .'&fkey_value="+fkey_value+"');
    ...
    public function set_subgrid($obj_grid, $d_fkey, $m_fkey=-1){
        if($obj_grid instanceof C_DataGrid){
            ...
            $obj_grid->set_jq_editurl($this->edit_file .'?dt='. $this->jq_datatype .'&gn='. $obj_grid->jq_gridName .'&src=sg&fkey='. $m_fkey .'&fkey_value="+fkey_value+"'); // fkey_value js variable is defined in display_subgrid(). the end " is necessary so it becomes +""
    ...
    public function set_col_edittype($col_name, $ctrl_type, $keyvalue_pair=null, $multiple=false, $dataUrl=null){
        if($ctrl_type == "select" || $ctrl_type == "autocomplete") {
            ...
            $data ="";
            $matches = array();
            if (is_array($keyvalue_pair)){
                $result = $keyvalue_pair;
                foreach($result as $i=>$val)
                {
                    $select_list.=$i.":".$val.";";
                }
                $select_list=rtrim($select_list,";");
                $keyvalue_pair = ($multiple?"":"null:;").$select_list; // ADODB_FORCE_VALUE - 3 force nul to Sql NULL
            }
            elseif (preg_match($regex , $keyvalue_pair, $matches))
            {
            ...
            $this->script_ude_handler .=
                'jQuery(document).ready(function($){ 
                    $(window).bind("resize", function() {
                                        phpGrid_'. $this->jq_gridName .'.setGridWidth($("window").width());
                                    }).trigger("resize");
                });' ."\n";
        }
...
// $col_d: dropdown detail
            ...
            setTimeout(function(){            
                ...
                $("#'. $this->jq_gridName .'_pivot").jqGrid("jqPivot",
                    ...
                    // grid options
                    {
                        ...
                        shrinkToFit: false,
						autowidth: ' . C_Utility::literalBool($this->jq_autowidth) .',
                        autoresizeOnLoad: '. C_Utility::literalBool($this->jq_autoresizeOnLoad) .',
                        pager: true,
	...
	public function test_after_submit(){
        $this -> add_event("jqGridAddEditAfterSubmit", "function (event, status, postData){console.log(event);console.log(status);console.log(postData);}");
